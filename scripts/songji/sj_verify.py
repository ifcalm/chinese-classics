# -*- coding: utf-8 -*-
"""宋人別集·零改字校驗。兩側不同源：底本側取源 JSON 的 title+paragraphs 拼漢字流
（**不走 tj_clean**），落盤側從 base-data 抽。判據：落盤漢字流須是底本漢字流的
子序列（只許刪、不許改、不許增）。
"""
import json, glob, os, re, sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from sj_build import BOOKS, DEST, files
...
def main():
    ...
    bad = 0
    print('%-16s %9s %9s %7s' % ('书', '底本', '落盘', '留存'))
    for b in BOOKS:
        s = ''.join(src[b['poet']])
        d = disk_stream(b['slug'])
        ok, pos = is_subseq(d, s)
        print('%-16s %9d %9d %6.1f%%  %s' % (
