...
# 卷一卷二已机器断句(逐块汉字保真校验),存断句稿 sanming-c<N>-punct.md,构建时径用,
# 白文重排逻辑仅用于卷三至卷十二;如此重跑 parse-shushu 不会以白文覆盖标点。
def build_sanming():
    ...
    write_book('san-ming-tong-hui', 'mingli', {
        ...
        'summary': '明万民英撰，十二卷。子平命理集大成之总汇，博采唐宋以来星命诸家格局、神煞、诗诀之说，考据宏富，为命学类书之渊薮，四库全书子部术数类著录。据四库全书本转录收录（白文，篇目仍旧，双行夹注作括注；维基整理本节录过甚故不用）。卷一、卷二为便读者，编者试加新式标点（逐字校勘保真，仅补标点未改一字），余卷仍四库白文原貌。'
    }, chs)
